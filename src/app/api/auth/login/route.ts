...
export async function POST(request: Request) {
  ...
  try { const result = getAuthService().login(input.data.username, input.data.password, getRequestClientKey(request)); return Response.json({ user: result.user }, { headers: { "Set-Cookie": buildSessionCookie(result.token, result.expiresAt) } }); }
