...
export interface ApiKeyRecord { id: string; ownerUserId: string; name: string; prefix: string; enabled: boolean; allowedModels: string[] | null; expiresAt: string | null; lastUsedAt: string | null; createdAt: string }
function apiKeyFromRow(row: Row): ApiKeyRecord { return { id:String(row.id),ownerUserId:String(row.owner_user_id),name:String(row.name),prefix:String(row.key_prefix),enabled:Boolean(row.enabled),allowedModels:row.allowed_models_json?JSON.parse(String(row.allowed_models_json)):null,expiresAt:nullableString(row.expires_at),lastUsedAt:nullableString(row.last_used_at),createdAt:String(row.created_at) } }
...
export class ApiKeyRepository {
  ...
  constructor(readonly ownerUserId:string,readonly db:AppDatabase=getDatabase(),hasher?:ApiKeyHasher){if(!ownerUserId)throw new Error("ownerUserId is required");this.hasher=hasher??new ApiKeyHasher(getSystemSecret(db,"api_key_pepper"))}
  create(name:string,allowedModels?:string[]|null,expiresAt?:string|null):ApiKeyRecord&{key:string}{const id=randomUUID(),key=this.hasher.generate(),timestamp=nowIso();this.db.prepare(`INSERT INTO api_keys(id,owner_user_id,name,key_prefix,key_hash,enabled,allowed_models_json,expires_at,created_at,updated_at) VALUES(?,?,?,?,?,1,?,?,?,?)`).run(id,this.ownerUserId,name,key.prefix,key.hash,allowedModels?JSON.stringify(allowedModels):null,expiresAt??null,timestamp,timestamp);return{id,ownerUserId:this.ownerUserId,name,prefix:key.prefix,key:key.plaintext,enabled:true,allowedModels:allowedModels??null,expiresAt:expiresAt??null,lastUsedAt:null,createdAt:timestamp}}
